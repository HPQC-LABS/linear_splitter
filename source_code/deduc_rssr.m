...
cMClique[m_]:=Times@@(a@@@Subsets[Array[c,m],{2}]);
flip[h_]:=Expand[h/.{a[b_,c_]:>1-a[b,c]}];
deducReduc[n_,N_]:=Times@@(a@@@#)->0&/@(Subsets[#,{2}]&/@Subsets[Array[c,N],{n}]);
cliqueTerm[m_,n_,N_]:=flip[flip[cMClique[m]]/.deducReduc[n,N]];
cliqueReduced[m_,n_,N_]:=If[N>=m,Total[cliqueTerm[m,n,N]/.(Thread[Array[c,m]->#]&/@Subsets[Range[N],{m}])],0];
indepent[m_,n_,N_]:=If[N>=n,Total[Expand[Times@@(1-a@@@Subsets[Array[c,n],{2}])]/.(Thread[Array[c,n]->#]&/@Subsets[Range[N],{n}])],0];
h[m_,n_,N_]:=indepent[m,n,N]+cliqueReduced[m,n,N];
